...
const checkAndPrintPrices = async () => {
    ...
    for (let market in dydxData) {
        if (dydxData[market].ticker) {
            ...
            let binanceTicker = dydxData[market].ticker.replace('-USD', 'USDT'); // '-USD'를 'USDT'로 변경

            // SHIB 또는 PEPE 티커인 경우 Binance 티커에 '1000' 접두사 추가
            ...
            try {
                ...
                if (binanceTicker === '1000SHIBUSDT' || binanceTicker === '1000PEPEUSDT'){
                    dydxPrice = dydxPrice * 1000
                }

                if (binancePrice) {
                    console.log(`Ticker: ${dydxTicker}, DYDX Price: ${dydxPrice}, Binance Price: ${binancePrice}`);
                } else {
                    console.log(`Ticker: ${dydxTicker} is not available on Binance Futures.`);
                }
            } catch (error) {
